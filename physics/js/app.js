...
function qzIntro() {
  ...
  document.getElementById('qzBody').innerHTML =
    ...
    '<div class="rw">You will get ' + Math.min(10, pool.length) + ' of them, shuffled.</div>' +
    '<div class="btnrow"><button class="btn" id="start">Start quiz</button></div></div>';
  document.getElementById('start').onclick = startQuiz;
}
function poolFor(f) {
  return f === 'all' ? QUESTIONS : QUESTIONS.filter(function (q) { return q.s === f; });
}
function dealQuiz() {
...
function chipRow(id, current, onPick) {
  var wrap = document.getElementById(id);
  if (!wrap) return;
  var items = [{ v:'all', label:'All sections' }].concat(SPEC.sections.map(function (s) {
