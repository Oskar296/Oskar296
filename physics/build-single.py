...
Useful for putting the whole app on a phone or a memory stick, or opening it
with no server and no internet connection. Output goes to
physics-4ph1-single.html.
...
def build():
    ...
    # Keep the order index.html loads them in: the data files fill in the
    # globals that app.js reads as soon as it runs.
    scripts = re.findall(r'<script src="\./js/([a-z0-9/-]+)\.js"></script>', html)
    if not scripts:
        sys.exit('found no script tags to inline; has index.html changed?')

    for name in scripts:
        js = read('js', *('%s.js' % name).split('/')).strip()
